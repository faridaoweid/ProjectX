...
type WindowActivityEntry = {
    status: ReactNode;
    time: string;
  };

  interface WindowActivityViewProps {
    windowActivities: WindowActivityEntry[];
...
const styles: Styles = {
  ...
  const WindowActivityView: React.FC<WindowActivityViewProps> = () => {
    const [windowActivities, setWindowActivities] = useState<WindowActivityEntry[]>([]);
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const reader = new FileReader();
            reader.onload = (e: any) => {
                ...
                const status = data.slice(1).map((row: any[]) => ({ TIME: excelTimeToHHMM(row[0]), STATUS: row[1] }));
    
                setWindowActivities(status.map(entry => ({
                  time: entry.TIME,
                  status: entry.STATUS,
              })));
  ...
  const fileInputLabelStyle: React.CSSProperties = {
    ...
    const buttonStyle: React.CSSProperties = {
      ...styles.button,
      backgroundColor: isUploadHovering ? 'darkblue' : 'blue',
    };
    ...
    return (
      <div>
        <h1 style={{ color: '#333', textAlign: 'center' }}>Upload your Window Activity Data</h1>
        ...
        <div style={styles.tableContainer}>
          <h2 style={{ color: '#333', textAlign: 'center' }}>Window Activity Data</h2>
          <table style={styles.table}>
            ...
            <tbody>
              {windowActivities.map((WindowActivityEntry: {time: string, status: ReactNode }, index) => (
                <tr key={index}>
                  <td style={styles.cell}>{WindowActivityEntry.time}</td>
                  <td style={{ ...styles.cell, textAlign: 'center' }}>{WindowActivityEntry.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  export default WindowActivityView;